...
Rcpp::DataFrame zbtyield(std::vector<QuantLib::Date> MatDates,
    ...
    for (QuantLib::Size i=0; i<numberOfBonds; i++) {
        quote.push_back(QuantLib::ext::make_shared<QuantLib::SimpleQuote>(cleanPrice[i]));
    }
    ...
    for (QuantLib::Size  j = 0; j< numberOfBonds;j++){
        ...
        QuantLib::ext::shared_ptr<QuantLib::FixedRateBondHelper> helper =
            QuantLib::ext::make_shared<QuantLib::FixedRateBondHelper>(quoteHandle[j], 1, faceAmount, schedule,
